...
SystemJSLoader.prototype.config = function(cfg) {
  ...
  for (var c in cfg) {
    ...
    if (['baseURL', 'map', 'packages', 'bundles', 'paths', 'warnings', 'packageConfigPaths', 'loaderErrorStack', 'browserConfig', 'nodeConfig', 'productionConfig'].indexOf(c) != -1)
      continue;
  ...
  if (cfg.nodeConfig && envModule.node)
    this.config(cfg.nodeConfig);
  if (cfg.browserConfig && envModule.browser)
    this.config(cfg.browserConfig);
  if (cfg.productionConfig && envModule.production)
    this.config(cfg.productionConfig);
};