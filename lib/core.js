...
hook('normalize', function(normalize) {
  return function(name, parentName) {
    ...
    if (normalized.substr(0, 6) == '@node/' && nodeCoreModules.indexOf(normalized.substr(6)) != -1) {
      if (!this._nodeRequire)
        throw new TypeError('Error loading ' + normalized + '. Can only load node core modules in Node.');
      this.set(normalized, this.newModule(getESModule(this._nodeRequire(normalized.substr(6)))));
    }
    
    // relative URL-normalization
    if (normalized[0] == '.' || normalized[0] == '/') {
      if (parentName)
        normalized = new URL(normalized, parentName.replace(/#/g, '%05')).href.replace(/%05/g, '#');
      else
