...
SystemJSLoader.prototype.config = function(cfg) {
  ...
  if (cfg.baseURL) {
    ...
    if (checkHasConfig(this.packages) || checkHasConfig(this.meta) || checkHasConfig(this.depCache) || checkHasConfig(this.bundles))
      throw new TypeError(errMsgWithLink.call(this, 'baseURL should only be configured once and must be configured first.'));
  ...
  if (cfg.map) {
    ...
    if (objMaps)
      dWarn.call(this, 'The map configuration for ' + objMaps + ' uses object submaps, which is deprecated in global map.\nUpdate this to use package contextual map with configs like System.config({ packages: { "' + p + '": { map: {...} } } }).');
  }
  ...
  for (var c in cfg) {
    ...
    if (c == 'baseURL' || c == 'map' || c == 'packages' || c == 'bundles' || c == 'paths' || c == 'deprecationWarnings' || c == 'packageConfigPaths')
      continue;
