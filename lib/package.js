...
(function() {
  ...
  // defaultExtension is only added if the path is not a loader
  // which we tell by doing an initial check of the meta first
  function toPackagePath(pkgName, pkg, basePath, subPath, defaultJSExtension) {
    var normalized = pkgName + '/' + basePath + subPath;

    var loaderModule = false;
    if (pkg.meta)
    ...
    return normalized + (loaderModule ? '' : getDefaultExtension(pkg, subPath, defaultJSExtension));
  }
