 ...
 * - map and defaultJSExtension are applied to the main
 * - defaultJSExtensions adds the '.js' extension only if no other extension is present
 * - if a map or meta value is available for a module without defaultJSExtensions, defaultJSExtensions is skipped
 * - like global map, package map also applies to subpaths (sizzle/x, ./vendor/another/sub)
 * - map targets do not pass through any further map or defaultJSExtensions
 *
  ...
  hook('normalize', function(normalize) {
    return function(name, parentName) {
      ...
      if (pkgName) {
        ...
        var mapped = applyMap(pkg.map, subPath) || 
          pkg.defaultJSExtensions && normalized.split('/').pop().indexOf('.') == -1 && applyMap(pkg.map, subPath + '.js');
        if (mapped)
          normalized = mapped.substr(0, 2) == './' ? pkgName + mapped.substr(1) : mapped;

        // apply default extension if not in meta
        else if (pkg.defaultJSExtensions && (!pkg.meta || !pkg.meta[subPath.substr(2)]))
          normalized += '.js';
      }

      // defaultJSExtensions is also supported as a top-level loader config
      if (this.defaultJSExtensions && normalized.split('/').pop().indexOf('.') == -1)
        normalized += '.js';
