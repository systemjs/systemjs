      ...
      if (!sync) {
        ...
        for (var i = 0; i < this.packageConfigPaths.length; i++) {
          ...
          var match = normalized.match(packageConfigPathsRegExps[p] || 
              (packageConfigPathsRegExps[p] = new RegExp('^(' + p.substr(0, p.lastIndexOf('*') + 1 || p.lastIndexOf('/')).replace(/\*/g, '[^\\/]+') + ')(\/|$)')));
          if (match) {
            pkgPath = match[1];
            pkgConfigPath = pkgPath + p.substr(p.lastIndexOf('*') + 1 || p.lastIndexOf('/'));
            break;
          }
        }
