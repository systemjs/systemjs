...
(function() {
  ...
  function validMapping(mapMatch, mapped, pkgName, path) {
    ...
    if (mapped.substr(0, mapMatch.length) == mapMatch && 
        path.length > mapMatch.length + (mapMatch[mapMatch.length - 1] != '/'))
      return false;
  ...
  function doStringMap(loader, pkg, pkgName, mapMatch, mapped, path, skipExtensions) {
    ...
    // never return a trailing slash for a mapped value
    // this allows map: { 'x': 'x/y' } to work with x/ properly
