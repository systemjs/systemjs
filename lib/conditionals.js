 ...
 */
  function resolveCondition(conditionModule, parentName, bool) {
    ...
    var booleanNegation = bool && conditionModule && conditionModule[0] == '~';
    if (booleanNegation)
    ...
    .then(function(m) {
      if (conditionExport)
        m = readMemberExpression(conditionExport, m);
      else if (typeof m == 'object' && m + '' == 'Module')
        m = m['default'];

      return booleanNegation ? !m : m;
    });
  }
  ...
  function booleanConditional(name, parentName) {
    ...
    var conditionModule = name.substr(booleanIndex + 2);
