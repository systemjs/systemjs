  ...
  hook('translate', function(translate) {
    return function(load) {
      ...
      .then(function(source) {
        ...
        if (loader.loadedTranspiler_Runtime === false) {
          if (load.name == loader.normalizeSync('traceur-runtime')
              || load.name == loader.normalizeSync('babel/external-helpers*')) {
            if (source.length > 100)
              load.metadata.format = load.metadata.format || 'global';

            loader.loadedTranspiler_Runtime = true;
          }
        }

        // detect transpiler runtime usage to load runtimes
        if (load.metadata.format == 'register' && loader.loadedTranspiler_Runtime !== true) {
          if (!__global.$traceurRuntime && load.source.match(traceurRuntimeRegEx)) {
            loader.loadedTranspiler_Runtime = loader.loadedTranspiler_Runtime || false;
