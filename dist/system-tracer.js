...
function trace(loader) {

  // only used to copy non-prototypal config
  function copy(obj) {
    var newObj = {};
    for (var p in obj) {
  ...
  // first clone the loader
  // this creates a context for loading plugins
  loader.pluginLoader = new LoaderPolyfill(loader);
  loader.pluginLoader.paths = copy(loader.paths);
  if (loader.map)
    loader.pluginLoader.map = copy(loader.map);
  if (loader.meta)
    loader.pluginLoader.map = copy(loader.map);
  if (loader.versions)
    loader.pluginLoader.versions = copy(loader.versions);
  if (loader.bundles)
    loader.pluginLoader.bundles = copy(loader.bundles);

  loader.pluginLoader.baseURL = loader.baseURL;
