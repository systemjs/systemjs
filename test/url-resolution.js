...
describe("Simple normalization tests", function () {
  it("Should resolve relative with protocol", function () {
    assert.equal(
      resolveIfNotPlainOrUrl("./x:y", "https://x.com/y"),
      "https://x.com/x:y"
    );
  });
  it("Should convert  into /", function () {
    assert.equal(
      resolveIfNotPlainOrUrl("./a\\b", "https://x.com/z"),
      "https://x.com/a/b"
    );
  });
  it("Should resolve relative windows paths", function () {
    assert.equal(
      resolveIfNotPlainOrUrl("./test.js", "file:///C:/some/path/"),
      "file:///C:/some/path/test.js"
    );
  });
  ...
  it("Should support resolving plain URI forms", function () {
    assert.equal(
      resolveIfNotPlainOrUrl("./asdf", "npm:lodash/"),
      "npm:lodash/asdf"
    );
  });
  it("Should support backtracking exactly to the base in plain URI forms", function () {
    assert.equal(
      resolveIfNotPlainOrUrl("../", "npm:lodash/asdf/y"),
      "npm:lodash/"
    );
  });
  it('Should support "." for resolution', function () {
    assert.equal(
      resolveIfNotPlainOrUrl(".", "https://www.google.com/asdf/asdf"),
      "https://www.google.com/asdf/"
    );
  });
  it('Should support ".." resolution', function () {
    assert.equal(
      resolveIfNotPlainOrUrl("..", "https://www.google.com/asdf/asdf/asdf"),
      "https://www.google.com/asdf/"
    );
  });
  it('Should support "./" for resolution', function () {
  ...
  it('Should leave a trailing "/"', function () {
    assert.equal(
      resolveIfNotPlainOrUrl("./asdf/", "file:///x/y"),
      "file:///x/asdf/"
    );
  });
  it('Should leave a trailing "//"', function () {
  ...
  it('Should support a trailing ".."', function () {
    assert.equal(
      resolveIfNotPlainOrUrl("../..", "path/to/test/module.js"),
      "path/"
    );
  });
});
...
import fs from "fs";
var testCases = eval(
  "(" + fs.readFileSync("test/fixtures/url-resolution-cases.json") + ")"
);
...
describe("URL resolution selected WhatWG URL spec tests", function () {
  testCases.forEach(function (test) {
    ...
    if (
      test.input.indexOf(":") !== -1 &&
      (test.failure || test.input !== test.href)
    )
      return;

    // we don't handle hashes
    if (test.input.indexOf("#") !== -1) return;

    // we don't handle C| automatically converting into file:///c:/
    if (test.input.indexOf("C|") !== -1) return;
    ...
    if (
      ...
      if (test.input.startsWith("../") || test.input.startsWith("/")) return;
    }
    ...
    it(
      'Should resolve "' + test.input + '" to "' + test.base + '"',
      function () {
        var failed = false;
        try {
          var resolved =
            resolveIfNotPlainOrUrl(test.input, test.base) ||
            (test.input.indexOf(":") !== -1 && test.input) ||
            resolveIfNotPlainOrUrl("./" + test.input, test.base);
        } catch (e) {
          failed = true;
          if (!test.failure)
            throw new Error(
              'Resolution failure, should have been "' + test.href + '"'
            );
        }
        if (test.failure && !failed)
          throw new Error("Should have failed resolution");
        if (!test.failure) assert.equal(resolved, test.href);
      }
    );
  });
});
