import {
  System as globalSystem,
  setBaseUrl,
  applyImportMap,
} from "../dist/system-node.cjs";
  ...
  describe("resolve", () => {
    it("provides a default base url if one is not specified", () => {
      assert.equal(
        ...
        pathToFileURL(process.cwd()) + path.sep + "foo.js"
      );
    });
  ...
  describe("import maps", () => {
    it("can load a module from the network", async () => {
      ...
      applyImportMap(System, {
        imports: {
          rxjs:
            "https://unpkg.com/@esm-bundle/rxjs@6.5.4-fix.0/system/rxjs.min.js",
        },
      });
